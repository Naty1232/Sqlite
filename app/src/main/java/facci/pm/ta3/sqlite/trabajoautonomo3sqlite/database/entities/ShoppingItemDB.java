...
public class ShoppingItemDB {
 ...
 // Para la insercion de datos se va utilizar el metodo inserElement

    public void insertElement(String productName) {
        //TODO: Todo el código necesario para INSERTAR un Item a la Base de datos
       //SE hace referencia la base de datos para ello se utiliza el metodo getWritableDatabase()
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        //creamos una instancia de la clase "ContentValues" que como su nombre indica es un almacenador de un conjunto de datos.
    ...
    //referencia la base de datos para ello se utiliza el metodo getWritableDatabase()
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        //eliminacion de la tabla
        db.delete(ShoppingElementEntry.TABLE_NAME,"",null);
        db.close();
    }
       //Usa el metodo update para la actualizacion
    public void updateItem(ShoppingItem shoppingItem) {
        //TODO: Todo el código necesario para ACTUALIZAR un Item en la Base de datos
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        //Se crea un conjunto de valores vacío utilizando el tamaño inicial predeterminado
        ContentValues values = new ContentValues();
        //
        values.put(ShoppingElementEntry.COLUMN_NAME_TITLE, shoppingItem.getName());
        db.update(ShoppingElementEntry.TABLE_NAME,values,ShoppingElementEntry._ID + " = "+ shoppingItem.getId(),null);
        db.close();

       //clearAllItems();
    }
        //Eliminacion por cada item
    public void deleteItem(ShoppingItem shoppingItem) {
        ...
        //db.update(ShoppingElementEntry.TABLE_NAME,values,strFilter,null);
        db.delete(ShoppingElementEntry.TABLE_NAME,strFilter,null);
        db.close();
    }
}
